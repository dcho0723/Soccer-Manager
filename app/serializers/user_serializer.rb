class UserSerializer < ActiveModel::Serializer
  ...
  def show_users_player_position
    if self.object.players.length != 0
      self.object.players.where(name: self.object.name)[0]["position"]
    end
  end
  def show_users_player_number
    if self.object.players.length != 0
      self.object.players.where(name: self.object.name)[0]["number"]
    end
  end
  def show_users_player_country
    if self.object.players.length != 0
      self.object.players.where(name: self.object.name)[0]["country"]
    end
  end
